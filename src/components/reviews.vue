<template>
  <div class="short-commit">
    ...
    <div class="short-commit-item" :key="index" v-for="(item,index) in reviews.reviews">
      <span>
      ...
      <span>
        <div>{{item && item.abstract}}</div>
        <div>
          <!-- <i class="iconfont icon-dianzan"></i> -->
          <span>{{item && item.comments_count}}回复</span>
          <span>•{{item && item.likers_count}}有用</span>
          <span>•{{item && item.likers_count}}转发</span>
        </div>
      </span>
    </div>
    <!-- <div class="more">
...
export default {
  ...
  mounted() {
    console.log("reviews", this.reviews.reviews);
  }
};
</script>
